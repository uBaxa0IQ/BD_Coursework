...
\begin{itemize}
    \item наличие встроенного процедурного языка PL/pgSQL, необходимого для
          реализации сложных математических алгоритмов расчёта продвинутых
          спортивных метрик на стороне базы данных;
...
через официальный API NBA Stats~\cite{nba-stats} и библиотеку
\texttt{nba\_api}~\cite{nba-api}, формирующую HTTP-запросы к сервису
\texttt{stats.nba.com}. Скрипт извлекает архивную
статистику NBA за пять регулярных сезонов 2019/20--2023/24.
...
\begin{table}[H]
\centering
\caption{Результат многотабличного запроса по матчам сезона 2023-24}
...
\begin{table}[H]
\centering
\caption{Средняя результативность по позициям (сезон 2023-24)}
...
\begin{table}[H]
\centering
\caption{Игроки с PER выше среднего по лиге (сезон 2023-24)}
...
\begin{table}[H]
\centering
\caption{Топ-5 лидеров по очкам и передачам (сезон 2023-24)}
...
Для защиты данных от аномалий реализован отложенный триггер ограничения
\texttt{trg\_check\_team\_score} (\texttt{DEFERRABLE INITIALLY DEFERRED}).
Он проверяет бизнес-правило предметной области: сумма очков всех игроков
команды в конкретном матче должна равняться итоговому командному счёту;
валидация выполняется на этапе \texttt{COMMIT}. Текст функции-триггера
приведён в приложении~Г (листинг~\ref{lst:trigger}), команда создания~---
в листинге~\ref{lst:trigger-ddl}.

Автоматический пересчёт витрины \texttt{player\_season\_stats} реализован
триггером \texttt{trigger\_update\_season\_stats} с переходной таблицей
\texttt{inserted\_rows} (листинг~\ref{lst:season-stats-trigger}).
...
с идентификатором~2327 за регулярный сезон 2023-24 с идентификатором~5.
