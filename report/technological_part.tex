...
Для выбора основной СУБД проведено сравнение трёх распространённых решений
по измеримым критериям (таблица~\ref{tab:dbms-compare}).
...
\hline
Поддержка PL/pgSQL (хранимые процедуры) & Да & Нет & Нет \\
\hline
Поддержка \texttt{DEFERRABLE}-триггеров & Да & Нет & Нет \\
\hline
Асинхронный драйвер Python (asyncpg) & Да & Нет & Нет \\
\hline
Поддержка UPSERT (\texttt{INSERT\ldots ON CONFLICT}) & Да & Да & Да \\
\hline
\end{tabular}
\end{table}

По совокупности критериев выбрана PostgreSQL~15~\cite{postgres}:
единственная из рассмотренных СУБД поддерживает все три ключевых механизма~---
PL/pgSQL, \texttt{DEFERRABLE}-триггеры и асинхронный драйвер asyncpg.
