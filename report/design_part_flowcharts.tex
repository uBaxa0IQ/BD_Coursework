...
\newcommand{\flowchartRightShiftSmall}{6.3cm} % компактная схема calculate_ts
...
\node (pro1)   [process,  below of=start] {$PTS, FGA, FTA :=$ суммы одноимённых \\ показателей игрока за сезон};
\node (pro2)   [process,  below of=pro1]
  {$D = 2 \cdot (\mathrm{FGA} + 0{,}44 \cdot \mathrm{FTA})$};
\node (dec1)   [decision, below of=pro2, yshift=-0.5cm] {$D = 0$?};
\node (pro3a)  [io,  right of=dec1, xshift=\flowchartRightShiftSmall] {Результат не определён};
...
\node (agg)    [process,  below of=start]
  {$FGM, FG3M, FGA :=$ суммы одноимённых \\ показателей игрока за сезон};
\node (dec1)   [decision, below of=agg] {$\mathrm{FGA} = 0$?};
\node (null1)  [io,  right of=dec1, xshift=\flowchartRightShift] {Результат не определён};
...
\node (null0)  [io,  right of=dec0, xshift=\flowchartRightShift] {Результат не определён};
\node (aggP)   [process,  below of=dec0, yshift=-0.2cm]
  {$pFGA, pFTA, pTOV, pMP :=$ суммы \\ одноимённых показателей игрока};
\node (dec1)   [decision, below of=aggP, yshift=-0.2cm] {$pMP < 1$?};
\node (null1)  [io,  right of=dec1, xshift=\flowchartRightShift] {Результат не определён};
\node (aggT)   [process,  below of=dec1, yshift=-0.2cm]
  {$tmFGA, tmFTA, tmTOV, tmMP :=$ суммы \\ одноимённых показателей команды};
...
\node (null2)  [io,  right of=dec2, xshift=\flowchartRightShift] {Результат не определён};
...
\node (start) [startstop] {Начало};
\node (agg)   [process,  below of=start]
  {$PTS, TRB, AST, STL, BLK, TOV, PF,$ \\ $FGM, FGA, FTM, FTA, MP :=$ суммы \\ одноимённых показателей игрока за сезон};
...
\node (start) [startstop] {Начало};
\node (agg)   [process,  below of=start]
  {$PTS, TRB, AST, STL, BLK, TOV, PF, MP$ \\ $:=$ суммы одноимённых показателей \\ игрока за сезон};
\node (dec1)  [decision, below of=agg] {$\mathrm{MP} < 50$?};
\node (null1) [io,  right of=dec1, xshift=\flowchartRightShift] {Результат не определён};
...
\node (skip)  [io,  right of=dec1, xshift=\flowchartRightShift] {Пропустить проверку};
...
\node (loop)  [predefined,  below of=exp]
  {Для каждой команды матча: \\ $teamPoints :=$ сумма очков её игроков};
\node (dec2)  [decision, below of=loop] {$teamPoints \neq expected$?};
\node (exc)   [io,  right of=dec2, xshift=\flowchartRightShift] {Отклонить транзакцию};
\node (ret)   [io,  below of=dec2] {Разрешить операцию};
\node (stop)  [startstop, below of=ret] {Конец};
...
\node (null1) [io,  right of=dec1, xshift=\flowchartRightShift] {Завершить выполнение};
\node (call)  [predefined,  below of=dec1, yshift=\flowchartTinyVExtra]
  {Для каждого season\_id: \\ Вызов процедуры \texttt{update\_season\_stats}};
\node (ret)   [io,  below of=call, yshift=\flowchartTinyVExtra] {Завершить выполнение};
\node (stop)  [startstop, below of=ret] {Конец};
...
\begin{tikzpicture}[
    ...
    startstop/.style={fc startstop},
    process/.style={fc process},
    process tall/.style={fc process, minimum height=1.35cm, inner sep=5pt},
    io/.style={fc io},
    arrow/.style={fc arrow}
]
\node (start) [startstop] {Начало};
\node (in)    [io, below of=start]
  {Получить данные из \texttt{game\_player\_stats} \\ и \texttt{games} (только завершённые матчи)};
\node (group) [process, below of=in]
  {Группировка статистики игроков \\ по (\texttt{team\_id}, \texttt{season\_id}, \texttt{game\_id})};
\node (agg)   [process tall, below of=group, yshift=-0.15cm]
  {$\mathrm{PTS}, \mathrm{REB}, \mathrm{AST}, \mathrm{TOV},$ \\ $\mathrm{FGM}, \mathrm{FGA}, \mathrm{FG3M}, \mathrm{FTM}, \mathrm{FTA}$ \\ $:=$ суммы одноимённых \\ показателей в матче};
\node (avg)   [process tall, below of=agg, yshift=-0.35cm]
  {$N :=$ число матчей команды \\ avg\_pts, avg\_reb, avg\_ast, avg\_tov $:=$ \\ суммы показателей $/ N$ \\ efg\_pct, ts\_pct $:=$ расчёт по сезонным суммам бросков};
\node (out)   [io, below of=avg, yshift=-0.25cm]
  {Возврат средних командных показателей};
...
\node (pro3)  [predefined, below of=pro2]
...
\node (stop)  [startstop, below of=pro5] {Конец};
