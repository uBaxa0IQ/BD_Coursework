...
\node (start)  [startstop] {Начало};
\node (agg)    [process,  below of=start] {$FGM, FG3M, FGA := \mathrm{SUM}(\ldots)$ \\ из \texttt{game\_player\_stats}};
...
\node (aggP)   [process,  below of=dec0, yshift=-0.2cm] {$pFGA, pFTA, pTOV, pMP :=$ \\ $\mathrm{SUM}(\ldots)$ игрока};
\node (dec1)   [decision, below of=aggP, yshift=-0.2cm] {$pMP < 1$?};
\node (null1)  [io,  right of=dec1, xshift=4.5cm] {Вывод:\\ значение не определено};
\node (aggT)   [process,  below of=dec1, yshift=-0.2cm] {$tmFGA, tmFTA, tmTOV, tmMP :=$ \\ $\mathrm{SUM}(\ldots)$ команды};
\node (denom)  [process,  below of=aggT, yshift=-0.2cm] {$denom := pMP \cdot (tmFGA + 0{,}44 \cdot tmFTA + tmTOV)$};
...
\node (ret)    [io,  below of=dec2, yshift=-0.2cm] {Возврат $\displaystyle\frac{(pFGA+0{,}44\cdot pFTA+pTOV)\cdot\frac{tmMP}{5}}{denom}$};
\node (stop)   [startstop, below of=ret] {Конец};
...
\begin{tikzpicture}[
    ...
    process/.style={draw, rectangle,
      minimum width=5.6cm, minimum height=0.8cm,
      align=center, fill=white},
    predefined/.style={draw, rectangle,
      minimum width=5.6cm, minimum height=0.8cm,
      align=center, fill=white,
      path picture={
...
\node (start) [startstop] {Начало};
\node (agg)   [process,  below of=start] {$PTS, TRB, AST, STL, BLK,$ \\ $TOV, PF, FGM/FGA, FTM/FTA, MP := \mathrm{SUM}(\ldots)$};
...
\node (lg)    [predefined,  below of=uper] {Вычисление lg\_aPER: \\ AVG(uPER) по игрокам с MP $> 100$};
...
\node (start) [startstop] {Начало};
\node (agg)   [process,  below of=start] {$PTS, TRB, AST, STL,$ \\ $BLK, TOV, PF, MP := \mathrm{SUM}(\ldots)$};
\node (dec1)  [decision, below of=agg] {$\mathrm{MP} < 50$?};
\node (null1) [io,  right of=dec1, xshift=5cm] {Вывод:\\ значение не определено};
...
\node (lg)    [predefined,  below of=raw] {Вычисление lg\_avg: \\ AVG(raw) по игрокам с MP $> 100$};
...
\node (exp)   [process,  below of=dec1] {$expected :=$ счёт команды \\ (\texttt{home\_score} или \texttt{away\_score})};
\node (loop)  [predefined,  below of=exp] {Для каждой команды в матче: \\ $teamPoints := \mathrm{SUM(points)}$};
...
\node (start) [startstop] {Начало};
\node (get)   [io,  below of=start] {Получить список уникальных season\_id \\ из новых строк (переходная таблица NEW)};
\node (dec1)  [decision, below of=get] {Список season\_id пуст?};
\node (null1) [io,  right of=dec1, xshift=4.5cm] {Вывод:\\ значение не определено};
\node (call)  [predefined,  below of=dec1] {Для каждого season\_id: \\ \texttt{CALL update\_season\_stats(season\_id)}};
...
\begin{tikzpicture}[
    node distance=1.2cm,
    every node/.style={font=\footnotesize},
    startstop/.style={draw, rectangle, rounded corners=10pt, minimum width=5.8cm,
      minimum height=0.75cm, align=center, fill=white},
    process/.style={draw, rectangle, minimum width=5.8cm, minimum height=0.75cm,
      align=center, fill=white},
    arrow/.style={thick,->,>=stealth}
]
...
\begin{tikzpicture}[
    node distance=1.1cm,
    every node/.style={font=\footnotesize},
    startstop/.style={draw, rectangle, rounded corners=10pt, minimum width=6.4cm,
      minimum height=0.75cm, text centered, fill=white},
    process/.style={draw, rectangle, minimum width=6.4cm, minimum height=0.75cm,
