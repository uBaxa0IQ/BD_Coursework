...
\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{../img/123.pdf}
...
Потенциальные ключи спроектированных таблиц приведены в таблице~\ref{tab:potential_keys}.

\begin{table}[H]
\caption{Потенциальные ключи спроектированных таблиц}
\label{tab:potential_keys}
\centering
\begin{tabular}{|p{0.28\textwidth}|p{0.62\textwidth}|}
\hline
\textbf{Таблица} & \textbf{Потенциальные ключи} \\
\hline
\texttt{positions} & \{\texttt{position\_id}\}, \{\texttt{code}\} \\
\hline
\texttt{seasons} & \{\texttt{season\_id}\}, \{\texttt{label}\} \\
\hline
\texttt{teams} & \{\texttt{team\_id}\}, \{\texttt{nba\_team\_id}\}, \{\texttt{name}\}, \{\texttt{abbreviation}\} \\
\hline
\texttt{players} & \{\texttt{player\_id}\}, \{\texttt{nba\_id}\} \\
\hline
\texttt{games} & \{\texttt{game\_id}\} \\
\hline
\texttt{player\_team\_history} & \{\texttt{history\_id}\} \\
\hline
\texttt{game\_player\_stats} & \{\texttt{stat\_id}\}, \{\texttt{game\_id}, \texttt{player\_id}\} \\
\hline
\end{tabular}
\end{table}

Для таблиц \texttt{positions}, \texttt{seasons}, \texttt{teams}, \texttt{players}, \texttt{games} и \texttt{player\_team\_history} все потенциальные ключи состоят из одного атрибута, поэтому частичная зависимость в них невозможна.

Таблица \texttt{game\_player\_stats} имеет составной потенциальный ключ, включающий идентификаторы матча и игрока. Неключевые атрибуты этой таблицы характеризуют результат конкретного игрока в конкретном матче. Ни один показатель не может быть определен только идентификатором игрока или только идентификатором матча. Атрибут команды также зависит от обеих частей ключа из-за возможности переходов игрока в течение сезона. Таким образом, частичные зависимости устранены, и схема соответствует требованиям второй нормальной формы.
