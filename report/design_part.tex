...
и откатывает вставку. Схема алгоритма представлена в Приложении~А
(рисунок~\ref{fig:algo-trg-score}).

Функция-триггер \texttt{trigger\_update\_season\_stats} обеспечивает
автоматическое обновление витрины \texttt{player\_season\_stats} после
каждой пакетной вставки в таблицу фактов. Определяет \texttt{season\_id}
по идентификатору последней вставленной игры и передаёт его в процедуру
\texttt{update\_season\_stats}. Если \texttt{season\_id} не удаётся
определить (таблица пуста), функция завершается без вызова процедуры.
...
\node (react) [layer, fill=blue!8] at (0, 0)
  {\textbf{Веб-клиент (SPA)}};

\node (api) [layer, fill=orange!8] at (6, 0)
  {\textbf{REST API сервер}};

\node (pg) [store, fill=green!8] at (12.5, 1.5)
  {\textbf{Реляционная СУБД}};

\node (redis) [store, fill=red!8] at (12.5, -1.5)
