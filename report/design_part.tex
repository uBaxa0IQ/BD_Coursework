...
обеспечена созданием B-tree индексов~\cite{connolly}:
\begin{enumerate}
...
Ограничение уникальности на \texttt{(game\_id, player\_id)} в таблице
\texttt{game\_player\_stats} автоматически создаёт B-tree индекс, покрывающий
запросы поиска по паре «матч~-- игрок». Все индексы построены на основе
сбалансированного дерева (B-tree)~--- структуры, обеспечивающей логарифмическую
...
отложенный триггер-ограничение, выполняющий проверку не при построчной
вставке, а в конце транзакции, когда статистика всех участников матча уже
загружена.
Триггер \texttt{trigger\_update\_season\_stats}, срабатывающий после вставки
на уровне оператора, с помощью механизма переходных таблиц извлекает
уникальные \texttt{season\_id} из пакета вставленных строк и для каждого
вызывает \texttt{update\_season\_stats}.
...
и инициирует откат транзакции в её конце. Схема алгоритма представлена в Приложении~А
...
таблиц триггер извлекает список уникальных \texttt{season\_id} из всего пакета
...
Триггер срабатывает на уровне оператора,
...
к хозяевам или гостям определяется через объединение с таблицей \texttt{games}
...
\draw [->, thick] ($(backend.south)+(0,-2.6)$) --
  node[above] {4. запрос выборки} ($(db.south)+(0,-2.6)$);
\draw [<-, thick, dashed] ($(backend.south)+(0,-3.5)$) --
  node[above] {5. результат выборки} ($(db.south)+(0,-3.5)$);
\draw [->, thick] ($(backend.south)+(0,-4.4)$) --
  node[above] {6. запись в кэш с TTL} ($(redis.south)+(0,-4.4)$);
...
реализована хранимыми процедурами на стороне СУБД; ограничения целостности закреплены
